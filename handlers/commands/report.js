...
module.exports = {
    ...
    execute: async (message, args, client) => {
        ...
        if (!rMember)
            return message.reply("Did You mention someone? No? Why Not just do p!report <mention> <reason>").then(m => m.delete({timeout: 5000}));

        if (rMember.hasPermission("BAN_MEMBERS") || rMember.user.bot)
            return message.channel.send("That Person Has Ban Permissions Don't worry!").then(m => m.delete({timeout: 5000}));
