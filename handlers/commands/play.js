...
module.exports = {
    ...
    execute: async function (client, message, args) {
        ...
        var serverQueue = message.client.queue.cache.get(message.guild.id);
        ...
        message.client.queue.cache.set(message.guild.id, queueConstruct);
        queueConstruct.songs.push(song);

        const play = async (song) => {
            const queue = message.client.queue.cache.get(message.guild.id);
            if (!song) {
                sendError(
                    "Leaving The Vc Nobody Is playing Music",
                    message.channel
                );
                message.guild.me.voice.channel.leave(); //If you want your bot stay in vc 24/7 remove this line :D
                message.client.queue.cache.delete(message.guild.id);
                return;
            }
            let stream = null;
            ...
            queue.connection.on("disconnect", () => message.client.queue.cache.delete(message.guild.id));
